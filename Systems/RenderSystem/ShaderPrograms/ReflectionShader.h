#ifndef _AT_RENDER_SYSTEM_SHADER_REFLECTION_SHADER_H_
#define _AT_RENDER_SYSTEM_SHADER_REFLECTION_SHADER_H_
#include "../GPUShader.h"

namespace AT {
	class ReflectionShader : public GPUShader {
	public:
		...
		SHADER_PARAMETER(Texture2D, SSRTexture)
		SHADER_PARAMETER(Texture2D, BRDFLutTexture)
		SHADER_PARAMETER(TextureCube, PrefilteredEnviromentMapTexture)
		SHADER_PARAMETER(RWTexture2D, OutputTexture)
		END_SHADER_PARAMETER_GROUP(ComputeGroup)

		BEGIN_SHADER_PARAMETERS(ReflectionShaderParameters)
		...
		END_SHADER_PARAMETERS(ReflectionShaderParameters)

		using Parameters = ReflectionShaderParameters;
		ReflectionShader(RHI::Shader cs, GPURootSignatureManager& root_signature_manager) : GPUShader(root_signature_manager.GetDevice()) {
			m_ComputeShader = cs;
		...
		void SetParameters(RHI::CommandList command_list, ShaderParameters* parameters) const {
			...
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 5, 1, &param->Compute->DepthTexture.srv);
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 6, 1, &param->Compute->SSAOTexture.srv);
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 7, 1, &param->Compute->SSRTexture.srv);
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 8, 1, &param->Compute->BRDFLutTexture.srv);
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 9, 1, &param->Compute->PrefilteredEnviromentMapTexture.srv);
			m_Device->WriteDescriptorTable(param->Compute->m_descriptor_table, 10, 1, &param->Compute->OutputTexture.uav);
