...
RWTexture2D<float4> gSaveReservoirBuffer : register(u2, space0);
...
void RayGen() {
    ...
    ReSTIR_DI_Sample output_sample;
    output_sample = asuint(gReservoirBuffer[index.xy].x);

    LightSample light_sample = GetLightSample(surface_interaction.Position, Lights[output_sample]);
    ...
    gReservoirBuffer[index.xy].y *= visibility;
    float3 L = surface_interaction.brdf.F(surface_interaction.wo, surface_interaction.Normal, light_sample.wi) * light_sample.L * 10.0f * abs(dot(light_sample.wi, surface_interaction.Normal)) * gReservoirBuffer[index.xy].y;
    gOutput[index.xy] = float4(L, 1.0f);
    gSaveReservoirBuffer[index.xy] = gReservoirBuffer[index.xy];
}
